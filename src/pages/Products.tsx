...
import { Loader2, Plus, Search, Pencil, Trash2, Package, AlertTriangle, TrendingUp } from "lucide-react";
import {
  ...
  const { data: products = [], isLoading } = useQuery({
    ...
    queryFn: async (): Promise<Product[]> => {
      const { data, error } = await supabase
        .from("products")
        .select("id, name, hsn_code, stock_qty, cost_price, selling_price, gst_rate")
